...
class AuthController {
  register = async (req, res, next) => {
    try{
      ...
      const result = await userService.register({login, password, passwordConfirmation, email});
  ...
  passwordResetConfirm = async (req, res, next) => {
    try{
      ...
      const result = await userService.passwordResetConfirm(newPassword, resetLink);
      return res.json(result);
    } catch(e) {
      next(e);
    }
