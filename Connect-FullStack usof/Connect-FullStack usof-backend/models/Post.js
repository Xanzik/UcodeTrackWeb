...
class PostModel {
	...
	async getPostByID(id) {
		try {
			...
			throw ApiError.BadRequest(
				'Error while getting a post by ID:',
				error,
			);
		}
	}
	...
	async updatePostCategories(postId, newCategories) {
		try {
			...
			if (newCategories && newCategories.length > 0) {
				let categoryIds =
					await categoryService.getCategoryIds(newCategories);
				for (const categoryId of categoryIds) {
					await connection.execute(
						'INSERT INTO post_categories (post_id, category_id) VALUES (?, ?)',
						[postId, categoryId],
			...
			throw ApiError.BadRequest(
				'Error by updating post categories:',
				error,
			);
		}
	}
	...
	async getLikesForPost(postId) {
		try {
			...
			throw ApiError.BadRequest(
				'Error while getting likes for post:',
				error,
			);
		}
	}
	...
	async createLike(postId, user) {
        ...
        `;
			...
			if (existingLike.length > 0) {
				return {
					message: 'Like already exists for this user and post',
				};
			}
        ...
        `;
			...
			throw ApiError.BadRequest(
				'Error while creating like for post:',
				error,
			);
		}
	}
	...
	async blockPost(id) {
		try {
			...
			throw ApiError.BadRequest('Failed to block post', error);
		}
	}
	...
	async deleteLike(postId, user) {
		try {
			...
			throw ApiError.BadRequest(
				'Error while deleting like for post:',
				error,
			);
		}
	}
